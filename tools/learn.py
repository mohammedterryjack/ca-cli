...
@click.option("--input", "-i", type=Path, required=True, help="Input Parquet file")
@click.option(
...
@click.option(
    ...
    help="Iterative = FFNN with Backprop. One-shot = FFNN with Pseudo-Inverse. Generative = char-GPT",
)
@click.option(
    "--weights",
    "-w",
...
@click.option(
    "--hidden-layers", "-h", type=int, multiple=True, default=DEFAULT_HIDDEN_LAYERS
)
@click.option(
    "--activation", "-a", type=click.Choice(["relu", "tan"]), default=DEFAULT_ACTIVATION
)
@click.option(
    "--binarisation-threshold", "-b", type=float, default=DEFAULT_BINARISATION
)
...
def learn_cli(
    ...
    if additional_input_features:
        if (
            not additional_input_features.is_file()
            or additional_input_features.suffix != ".parquet"
        ):
            raise click.ClickException(
                f"Additional Input must be a Parquet file, got {additional_input_features}"
            )
        loaded_additional_evolutions = load_evolutions_from_parquet(
            additional_input_features
        )
    else:
    ...
    for name, (X_img, Y_img) in evo_per_image.items():
        ...
        if learn == "generative":
            ...
            try:
                Y_hat = safe_string_to_evolution(
                    Y_raw, decode_special=additional_input_features is not None
                )
                if not any(Y_hat):
    ...
    save_evolutions_as_parquet(
        evolutions=predicted_evolutions_to_save, output_dir=output
    )
