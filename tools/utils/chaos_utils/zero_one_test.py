...
class O1TestForChaos:
    @staticmethod
    def _transform(
        observables: List[float], angle: float
    ...
    @staticmethod
    def transform(
        observables: List[float], angle: float, path_save: Path | None = None
    ) -> List[complex]:
        z = cumsum(
            list(O1TestForChaos._transform(observables=observables, angle=angle))
        )
        if path_save:
            clf()
            figure()
    ...
    def _mean_square_displacement(
        transformed_data: List[complex], N: int
    ...
    def mean_square_displacement(
        transformed_data: List[complex], N: int, path_save: Path | None = None
    ) -> List[float]:
        M = list(
            O1TestForChaos._mean_square_displacement(
    ...
    def correlation_coefficient(
        mean_square_displacement: List[float], N: int, path_save: Path | None = None
    ) -> float:
        log_M = list(
            map(lambda M_n: log(M_n) if M_n else 0.0, mean_square_displacement)
        )
    ...
    @staticmethod
    def _test_for_chaos(
        observables: List[float], n_angles: int, N: int, path_save: Path | None = None
    ) -> Generator[float, None, None]:
        for n in range(1, n_angles + 1):
            c = (n / 20) * pi
            z = O1TestForChaos.transform(
                observables=observables,
                angle=c,
                path_save=path_save if n == 1 else None,
            )
            M = O1TestForChaos.mean_square_displacement(
                transformed_data=z, N=N, path_save=path_save if n == 1 else None
            )
            K = O1TestForChaos.correlation_coefficient(
                mean_square_displacement=M, N=N, path_save=path_save if n == 1 else None
            )
            yield K
    ...
    def test_for_chaos(
        ...
        Ks = list(
            O1TestForChaos._test_for_chaos(
                observables=observables,
                n_angles=n_angles,
