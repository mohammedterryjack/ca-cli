...
def evolution_to_1d_sequence_cosine(evolution: ndarray) -> list[float]:
    _, W = evolution.shape
    ref = make_reference_vector(width=W, mode="sin")
    trajectory = [embed_row_cosine(row=row, u=ref) for row in evolution]
    pmin = min(trajectory)
    pmax = max(trajectory)
...
def evolution_to_1d_sequence_gray(evolution: ndarray) -> list[float]:
    ...
    total = (max_cell_state + 1) ** evolution.shape[
        1
    ]  # number of possible states per row
