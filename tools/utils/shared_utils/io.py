...
def parquet_string_to_evolution_and_emergence(parquet_string: str) -> tuple[list[list[int]], list[list[bool]]]:
...
def safe_string_to_evolution(generated_string: str, row_length:int|None=None, decode_special:bool=False) -> list[list[int]]:
    generated_string = (
        generated_string.strip(BOS).replace(BOS, EOL).replace(WILDCARD, "")
    )
    # print(generated_string.replace(EOL,'\n'))
    if decode_special:
        evolution,_ = parquet_string_to_evolution_and_emergence(parquet_string=generated_string)
    else: 
...
def save_evolutions_as_parquet(
    evolutions: dict[str, ndarray], output_dir: Path,
    ...
    if additional_features is None:
        ...
        evolution_strings = [
            evolution_and_emergence_to_parquet_string(
               evolution=evolutions[name],
               emergence_mask=additional_features[name] 
            ) 
            for name in names 
        ]  
